<template>
  <header class="flex flex-row items-center w-full relative bg-blue-950 border-b-2 border-gray-400 max-sm:justify-between max-sm:relative h-20">
   <div class = "basis-1/4 items-center justify-starts px-8 min-w-40 max-sm:basis-auto">
        <img src="/assets/images/image.png" class = "w-[70px] h-[70px] rounded-xl bg-transparent">      
    </div>
    <div class = "basis-1/4 max-sm:basis-auto">
    </div>
    <nav :class="{'basis-1/2 text-yellow-500 flex flex-row items-center justify-end px-8 gap-4': !burger||burger,
    ...
    <div v-else class="max-sm:flex max-sm:flex-col max-sm:mr-8 max-sm:justify-between max-sm:items-center max-sm:w-8 max-sm:h-6" @click="switch_burger">
        ...
        <span class="h-[3px] w-full bg-yellow-500 relative bottom-3 rotate-[-45deg]"></span>
    </div>
</header>
    
<main class = "p-5 flex bg-[url(/assets/images/donpollobg.jpg)] bg-cover h-screen text-white">    
        <slot />
</main>

   <footer class = "flex flex-row w-full bg-blue-950 border-t-2 border-gray-400 items-center justify-center px-4 py-2 gap-4">
